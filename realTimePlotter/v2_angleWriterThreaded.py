...
with open('angle.csv', 'w') as csv_file:
    csv_writer = csv.DictWriter(csv_file, fieldnames=FIELD_NAMES)
    csv_writer.writeheader()


def writeAccuracy(accuracyDict):
...
def writeAngle(angleDict):
    with open('angle.csv', 'a') as csv_file:
        csv_writer = csv.DictWriter(csv_file, fieldnames=FIELD_NAMES)
        # print(angleDict)
...
def start():
    ...
    writeDataThread = threading.Thread(target=writeData)
